...
template<typename TParam, typename List, typename UNIQUE = NullType, int N = 4096>struct QuerySQL_CreateTable
{	
		QuerySQL_CreateTable(TParam *param)
		{
			 ...
			 SetUnicue<UNIQUE>()(buf);
			 wcscat(buf, L")");
		}
		wchar_t *operator()(){return buf;}
private:
	    wchar_t buf[N];
		template<class O, class P>struct __SetUnicue__
		{
		...
		template<class T>struct SetUnicue
		{
			void operator()(wchar_t *d)
			{
				wcscat(d, L",UNIQUE(");
				wchar_t *s = &d[(wcslen(d))];
				TL::foreach<UNIQUE, __SetUnicue__>()((TL::Factory<UNIQUE> *)0, s);
				s[wcslen(s) - 1] = ')';
			}
		};
		template<>struct SetUnicue<NullType>
		{
			void operator()(wchar_t *d) {}
		};
...
template<typename SubList, typename SetDefault = NullType>class CreateDataBase
{
	template<typename O, typename P>struct init
	{
		...
		{
			p->ConnectionSQL(
			    QuerySQL_CreateTable<O, O::items_list, O::unique_list, 1024>(o)()
			);
		}
	};
public:
