...
{
	double deltaTickDigit_ = deltaTickDigit / 50;
	int offsX = offs;
...
void LineSeries::Draw()
{
  if(NULL != data)
  {
    ...
    double y = chart.rect.bottom - chart.offsetAxesBottom  + dY * chart.minAxesY;
	...
	{
		if((dimention_of(points) - 2) < last) break;
		x += dX;
		int k = i;
		points[k].X = int(x + x0);
		if(last == k)
	...
	int len = last < count ? last : count;
	chart.g->DrawLines(&pen, points, len);
...
void LineSeries::Draw()
{
  if(NULL != data)
  {
	...
	//chart.g->DrawLines(&pen, points, len);
	//double x00 = x0;
	//double y00 = y - data[0] * dY;

	int x000 = (int)x0;
	int y000 = (int)(y - data[0] * dY);

	double x11 = x0;

	for(int i = 0; i < count; ++i)
	{
		double y11 = y - data[1 + i] * dY;
		x11 += dX;
		
		int x111 = (int)x11;
		int y111 = (int)y11;

		if(x000 != x111 || y000 != y111)
		{
			chart.g->DrawLine(&pen, x000, y000, x111, y111);
			x000 = x111;
			y000 = y111;
		}
	}
...
//, valueX(0)
//, oDiff(NULL), ptrDiff(NULL)
//, dxScaleDown(0), dyScaleDown(0)
//, dxScaleUp(0), dyScaleUp(0)
//, mouseDown(false)
{}	
