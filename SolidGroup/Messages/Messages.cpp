...
namespace EventLogSpace
{
	...
	{
		const unsigned &bit;
		__set_to_event_log_input__(): bit(Singleton<InputsTable>::Instance().items.get<O>().value){}
		bool operator()(O *, P *)
		{			
			if(tmp & bit)
			{
	...
	{
		const unsigned &bit;
		__set_to_event_log_output__(): bit(Singleton<OutputsTable>::Instance().items.get<O>().value){}
		bool operator()(O *, P *)
		{			
			if(tmp & bit)
			{
