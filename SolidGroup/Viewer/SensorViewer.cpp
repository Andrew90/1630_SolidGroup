...
bool SensorViewer::CursorLabel::Draw(TMouseMove &l, VGraphics &g)
{
	...
	if(leftOffs >= 0 && (data.countZones >= leftOffs - 1))
	{
		double y = data.zones[leftOffs - 1];
		sprintf(buf, "<0xff>Зона <0xff0000>%d   <0xff>толщина <0xff0000>%0.1f  %s "
			, leftOffs
			, y
			, StatusLabel(data.status[leftOffs - 1]).text
			);
		RECT r;
...
bool SensorViewer::CursorLabel::GetColorBar(int zone, double &data_, unsigned &color)
{
	if(zone == 0) return true;
	data_ = data.zones[zone - 1];
	ColorBar(data_, color, data.status[zone - 1]);
	return (int)zone <= data.countZones + 1;
}
#else
bool SensorViewer::CursorLabel::Draw(TMouseMove &l, VGraphics &g)
{
	...
	*/
	/*
	int status = currentAutomat->compute.sensorsData[num].status[leftOffs - 1];
	if(status)
	{
		char *txt = StatusLabel(status).text;
		if(status == PrimaryData::Nominal || status == PrimaryData::DefectMax || status == PrimaryData::DefectMin || status == PrimaryData::DefectMinMax)
		{
			double yMin = currentAutomat->compute.sensorsData[num].zonesMin[leftOffs - 1];
			double yMax = currentAutomat->compute.sensorsData[num].zonesMax[leftOffs - 1];
			double delta = currentAutomat->compute.thicknessData.zonesMax[leftOffs - 1] - currentAutomat->compute.thicknessData.zonesMin[leftOffs - 1];
...
bool SensorViewer::CursorLabel::GetColorBar(int zone, double &data_, unsigned &color, double &data_1, unsigned &color1)
{
	if(zone)
	{
		/*
		int i = zone - 1;
		data_1 = currentAutomat->compute.sensorsData[num].zonesMin[i];
		data_ = currentAutomat->compute.sensorsData[num].zonesMax[i];
		ColorBar(data_, color1, currentAutomat->compute.sensorsData[num].status[i]);
		...
		return 0 != currentAutomat->compute.sensorsData[num].status[i];
		*/
	}
	data_ = 0;
	return true;
...
template<class O, class P>struct __sent_mouse_move__
{
	...
	{
		p->hwnd = o->hWnd;
		o->cursor.VerticalCursor(*p, HDCGraphics(o->hWnd, o->backScreen));
		o->storedMouseMove = *p;
	}
};
...
//----------------------------------------------------------------------------------------------------
void SensorViewer::operator()(TDestroy &l)
{
...
template<class O, class P>struct sensor_viewer_button_dbl_clk__
{
	void operator()(O *o, P *p)
	{
		o->mouseMove = true;
		p->hwnd = o->hWnd;
		o->cursor.VerticalCursor(*(TMouseMove *)p, HDCGraphics(o->hWnd, o->backScreen));
...
void SensorViewer::operator()(TLButtonDbClk &l)
{
	...
	int leftOffs = int(double(l.x - x) / dX);
	/*
	if(currentAutomat->compute.sensorsData[num].status[leftOffs - 1])
	{
